...
class QuestionStem extends React.Component {
  ...
  onQuestionComplete = () => {
    ...
    if (questionInfo.NextQuestionId) {
      this.props.saveAnswerInfo &&　this.props.saveAnswerInfo().then(() => {
        this.props.changeQuestion && this.props.changeQuestion(questionInfo.NextQuestionId)
      });
    } else {
  ...
  handleHandInExam = () => {
    ...
    layer.confirm('当前已为最后一题，你已完成考试，请交卷。', {
      ...
      that.props.saveAnswerInfo &&　that.props.saveAnswerInfo().then(that.handInExam);
    });
  };
          ...
          <Row type="flex" className={styles.flexWrap}>
            <Col className={styles.infoHeader} span={24}>
              <Row className={styles.header} type="flex">
                ...
                <Col className="font-red text-r" span={12}>
                  {
                    questionInfo.AllowTimeLength !== "" ?
                      <CountDown count={parseInt(questionInfo.AllowTimeLength)}
                                 id={questionInfo.QuestionId}
                                 reload={this.state.reload}
                                 onComplete={this.onQuestionComplete}/> : null
                  }
                </Col>
              </Row>
            </Col>
